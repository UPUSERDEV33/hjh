				        ...
				        var img = new Image
				  		...
				  		r1.get(url).then(res => {
										...
										img.onload = function() {
															...
															ctx.save();
											    		ctx.beginPath();
											    		ctx.arc(54, 103, 47, 0, Math.PI * 2, true); //  
													    ctx.closePath();
													    ctx.clip();
													    ctx.drawImage(img, 8, 57, 92, 92); // 
													    ctx.restore();
										}
										img.src = dataURL;
						});
	    ...
	    if(cmd == '') {
		    ...
		    fs.readFile(__dirname + `/${wallpapers[args[0]].src}`, function (err, Background) {
		      ...
		      setTimeout(() => {
			    ...
			    .setFooter(`Tip: To preview wallpaper try \`${prefix}\``)
		    ...
		    Jimp.read(url, (err, ava) => {
			        ...
			        ava.getBuffer(Jimp.MIME_PNG, async (err, buf) => {
					     ...
					     ctx.drawImage(ava, 8, 43, 80, 85); //  
					        ...
					        //ur name
					        ctx.font = 'bold 16px profile'; //    
					        ctx.fontSize = '40px'; //  
					        ctx.fillStyle = "#FFFFFF"; //  
					        ctx.textAlign = "left"; //   
					        ctx.fillText(`${getvalueof.username}`, 100, 125) //  
					
					         //bord
					         let leaderboard = await SQLite.all(`SELECT * FROM profileSystem ORDER BY xp DESC, credits DESC`);
					        ctx.font = "regular 12px profile" //   
					        ctx.fontSize = '50px'; //  
					        ctx.fillStyle = "#FFFFFF" //  
					        ctx.textAlign = "left"; //  
					        ...
					        //credit
					        ctx.font = "bold 10px profile" //   
					        ctx.fontSize = '10px'; //  
					        ctx.fillStyle = '#FFFFFF' //  
					        ctx.textAlign = "left"; //   
					        ctx.fillText(`$ ${res.credits}`, 156, 163) //  
					
					        //poits
					        ctx.font = "bold 13px profile" // 
					        ctx.fontSize = '10px'; //    
					        ctx.fillStyle = "#FFFFFF" //  
					        ctx.textAlign = "left"; //   
					        ctx.fillText(`${res.xp}`, 173, 182) //  
					        ...
					        ctx.fillStyle = "#FFFFFF" //  
					        ctx.textAlign = "left"; //   
					        ctx.fillText(`${res.level}`, 30, 200) //  
					
					        //info
					        ctx.font = "blod 13px profile" // 
					        ctx.fontSize = '10px'; //    
					        ctx.fillStyle = "#FFFFFF" //  
					        ctx.textAlign = "left"; //   
					        ctx.fillText(`${res.info}`, 118, 40) //  
